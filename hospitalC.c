...
int main(int argc, char *argv[]){
    printf("Hospital C is up and running using UDP on port 32040\n");
    // read in map
    ...
    int orig_loc = atoi(argv[1]); // hospital C location#
    int rein_loc; // reindexed C location#
    for (int k = 0; k < 100; k++) {
    ...
    bind(c_socket, (struct sockaddr*) &c_address, sizeof(c_address));
    ...
    sendto(c_socket, &info, sizeof(info), 0, (struct sockaddr*) &scheduler_address, len);
    ...
    while (1) {
        ...
        int temp;
        recvfrom(c_socket, &temp, sizeof(temp), 0, (struct sockaddr*) &scheduler_address, &len);
        ...
        sendto(c_socket, &dist, sizeof(dist), 0, (struct sockaddr*) &scheduler_address, len);
        if (rein_pat_loc == -1) {
            sendto(c_socket, &score, sizeof(score), 0, (struct sockaddr*) &scheduler_address, len);
