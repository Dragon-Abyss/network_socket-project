...
int main(int argc, char *argv[]){
    printf("Hospital A is up and running using UDP on port 30040\n");
    // read in map
    ...
    int orig_loc = atoi(argv[1]); // hospital A location#
    int rein_loc; // reindexed A location#
    for (int k = 0; k < 100; k++) {
    ...
    //create UDP socket
    int a_socket;
    a_socket = socket(PF_INET, SOCK_DGRAM, 0);
    ...
    bind(a_socket, (struct sockaddr*) &a_address, sizeof(a_address));
    ...
    sendto(a_socket, &info, sizeof(info), 0, (struct sockaddr*) &scheduler_address, len);
    ...
    while (1) {
        ...
        int temp;
        recvfrom(a_socket, &temp, sizeof(temp), 0, (struct sockaddr*) &scheduler_address, &len);
        ...
        sendto(a_socket, &dist, sizeof(dist), 0, (struct sockaddr*) &scheduler_address, len);
        if (rein_pat_loc == -1) {
            sendto(a_socket, &score, sizeof(score), 0, (struct sockaddr*) &scheduler_address, len);
